...
def moving_average(x, w):
    ...
    print('in mv {:.8f}'.format(time.time()-start))
    return result
...
def video_capture(cap, frame_queue, darknet_image_queue, frame_rgb_queue, pose_queue, fps_skip, time_queue, rotate_ang, bg_mask, frame_queue_toShowing):
    print(type(cap))
    frame_count = 0
    while cap.isOpened():
        ret, frame = cap.read()
...
def get_delta_xy(arr, _dis=-1, _thres=-0.9):
    ...
    # print(sim)
    if abs(secondVector[0] - firstVector[0]) > 10:
        sim = cosine_similarity([firstVector[:2]], [secondVector[:2]])
...
def drawing(frame_queue, detections_queue, fps_queuei, mask, frame_rgb_queue, draw_original, time_queue, t_fun, segRally_thres, image_queue, tail_queue):
    noTail_count = 0    
    import time
...
def showing(cap, out_filename, image_queue, hand_queue, mask, tail_queue, t_fun, draw_original, name1, name2, watermark_img, frame_queue_toShowing):
    if draw_original:
        h, w,_ = mask.shape
...
def pose_estimate(op, opwrapper, pose_queue, kp_queue, skeleton_mask):
    ...
    while True:
        ...
        if frame is not None:
            ...
            kp_queue.put((idx, keypoints.astype(np.float16)))
            if kp_queue.full():
                kp_queue.get()
    cap.release()
...
def wait_for_hit_point(kp_queue, hitpoint_queue, hand_queue, left_handed):
    while True:
        if hitpoint_queue.empty():
            ...
            print('whole judge', time.time()-start)
            print('--'*30)
...
def judge_fore_back(kp_queue, hit_point, left_handed=(False, False)):
    ...
    while not kp_queue.empty():
        ...
        count += 1
    print('get kp {:.8f}'.format(time.time()-start))
    ...
    for idx, points in keypoints:
        ...
        if (not right and is_left) or (right and not is_left):
            ...
            person = points[1]
            arm_kps.append((idx, person[arm_side, :]))
    print('get person {:.8f}'.format(time.time()-tic))
    print('big {:.8f}'.format(time.time()-big_start))
    angles = []
    frame_idx = []
    start = time.time()
    ...
    print('calculate angle {:.8f}'.format(time.time()-start))
    ...
    if np.isnan(angles).any():
        ...
        s = s.interpolate()
        angles = s.tolist()    
    print('interpolate {:.8f}'.format(time.time()-start))
    
    start = time.time()
    angles = moving_average(angles, 5)
    print('mv {:.8f}'.format(time.time()-start))
    # print('mv', angles)
    # print(frame_idx[0], frame_idx[-1])
    # print(len(angles))
    start = time.time()
    is_forhand = fore_or_back(angles)
    print('fore or back {:.8f}'.format(time.time()-start))
...
if __name__ == '__main__':
    ...
    # Thread(target=video_capture, args=(frame_queue, darknet_image_queue, frame_rgb_queue, pose_queue, args.fps_skip, time_queue, args.rotate, bg_mask, frame_queue_toShowing)).start()
    # Thread(target=inference, args=(darknet_image_queue, detections_queue, fps_queue)).start()
    # Thread(target=drawing, args=(frame_queue, detections_queue, fps_queue, mask, frame_rgb_queue, args.draw_original, time_queue, t_fun, segRally_thres, image_queue, tail_queue)).start()
    # Thread(target=showing, args=(args.out_filename, image_queue, hand_queue, mask, tail_queue, t_fun, args.draw_original, name1, name2, watermark_img, frame_queue_toShowing)).start()
    ...
    print(cap.isOpened())
    p_cap = Process(target=video_capture, args=(cap, frame_queue, darknet_image_queue, frame_rgb_queue, pose_queue, args.fps_skip, time_queue, args.rotate, bg_mask, frame_queue_toShowing,))
    # p_inference = Process(target=inference, args=(darknet_image_queue, detections_queue, fps_queue,))
    # p_draw = Process(target=drawing, args=(cap, frame_queue, detections_queue, fps_queue, mask, frame_rgb_queue, args.draw_original, time_queue, t_fun, segRally_thres, image_queue, tail_queue,))
    # p_show = Process(target=showing, args=(args.out_filename, image_queue, hand_queue, mask, tail_queue, t_fun, args.draw_original, name1, name2, watermark_img, frame_queue_toShowing,))
    # p_pose = Process(target=pose_estimate, args=(op, opWrapper, pose_queue, kp_queue, skeleton_mask,))
    # p_hit = Process(target=wait_for_hit_point, args=(kp_queue, hitPoint_queue, hand_queue, left_handed,))
