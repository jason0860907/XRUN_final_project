            ...
            if (bounce is not None) and (np.sum(self.mask[int(bounce[1]), int(bounce[0])]) > 0) \
                ...
                if self.new_bounce is not None:
                    ...
                    if not report_hand_hit_queue.empty():
                        # print('not report_hand_hit_queue.empty()')
                        report_bounce_queue.put(bounce)
                        # print('report_bounce_queue.put(bounce)')
                # print('383: report_bounce_queue',report_bounce_queue.qsize(),' report_hand_hit_queue',report_hand_hit_queue.qsize())
                self.new_bounce = bounce
                np.save('bounce', self.bounce)
            # else:
            #     if report_bounce_queue.qsize()==0 and report_hand_hit_queue.qsize()==1:
            #         import pdb
            #         pdb.set_trace()
...
def report(report_hand_hit_queue, report_bounce_queue):
    ...
    while cap.isOpened():
        ...
        if len(report_array) > 0:
            ...
            for j in [0,5]: # side
                ...
                for h in [0,1,2]: # hand
                    ...
                    for i in range(j, j+5): # hit
                        ...
                        img_hm = copy.deepcopy(img_2d)
                        if h < 2:
                            _bounce_pts = bounce_pts[(np.array(report_array)[:,1]==i)* (np.array(report_array)[:,0]==h)]
                        else:
                    ...
                    # pdb.set_trace()
                    if j == 5:
                        _bounce_pts = bounce_pts[(np.array(report_array))[:,1]>4]
                        img_hm, x_y_cunt_patch = get_heatmap(img_2d, _bounce_pts, 
                                                    x0, cx, y0, y1, color_rg='g', x_patch=2, y_patch=3, returnImg=False) # patchB
                    else:
                        _bounce_pts = bounce_pts[(np.array(report_array))[:,1]<=4]
                ...
                # if i == 4: # new side
                #     _dict_side[_side_key_arr[0]]=_dict
                #     _dict = dict()

                # if i == 9: # new side
                #     _dict_side[_side_key_arr[1]]=_dict
                # print('_dict:', _dict)
                # pdb.set_trace()

                # img_name = 'imgs/temp/v4/report_211014_{}.png'.format(i)
                # cv2.imwrite(img_name, img_hm)
            ...
            # upload "upload.png" and renamed "upload.png"
            # storage.child('{}.jpg'.format(i+j)).put(img_name)
