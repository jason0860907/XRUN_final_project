    ...
    def inQ(self, bboxes, report_bounce_queue, report_hand_hit_queue):
        ...
        if self.count == 1:
            ...
            if hitPoint is not None:
                ...
                self.hitPoint_range_num = get_hit_point_range(self.hitPoint_range, hitPoint, table_h, table_w, win_h, win_w, t_fun)
...
def drawing(frame_queue, detections_queue, fps_queuei, mask, frame_rgb_queue, draw_original, 
...
def showing(out_filename, image_queue, hand_queue, mask, tail_queue, t_fun, draw_original, name1, name2, watermark_img, frame_queue_toShowing, report_hand_hit_queue):
    if draw_original:
        h, w,_ = mask.shape
...
def report(report_hand_hit_queue, report_bounce_queue):
    ...
    while cap.isOpened():
        ...
        if len(report_array) > 0:
            ...
            for i in range(10):
                img_hm = copy.deepcopy(img_2d)
                _bounce_pts = bounce_pts[np.array(report_array)[:,1]==i]
                # print(_bounce_pts)
                #pdb.set_trace()
                if i > 4:
                    img_hm, x_y_cunt_patchB = get_heatmap(img_2d, _bounce_pts, 
                                            x0, cx, y0, y1, color_rg='g', x_patch=2, y_patch=3)
                    img_hm = put_hm_text_h(img_hm, x_y_cunt_patchB)
                else:
                    img_hm, x_y_cunt_patchA = get_heatmap(img_hm, _bounce_pts, 
                                            cx, x1, y0, y1, color_rg='r', x_patch=2, y_patch=3)

                    img_hm = put_hm_text_h(img_hm, x_y_cunt_patchA)
    
                cv2.imwrite('imgs/temp/v3/report_211009_{}.png'.format(i), img_hm)
            #pdb.set_trace()
                #-----#
    cap.releace()
...
if __name__ == '__main__':
    ...
    # skeleton mask
    skeleton_mask = cv2.imread('./imgs/skeleton_mask.jpg', cv2.IMREAD_GRAYSCALE)
    ...
    Thread(target=showing, args=(args.out_filename, image_queue, hand_queue, mask, tail_queue, t_fun, args.draw_original, name1, name2, watermark_img, frame_queue_toShowing, report_hand_hit_queue)).start()
