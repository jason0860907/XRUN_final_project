...
def video_capture(frame_queue, darknet_image_queue, frame_rgb_queue, pose_queue, fps_skip, time_queue, rotate_ang, bg_mask, 
    frame_queue_toShowing, trigger_report_queue):
    frame_count = 0
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
        ...
        if (float(frame_count)/fps_skip)%30 == 0: # 30 is fps
            trigger_report_queue.put(True)
        frame_count += 1
...
def showing(out_filename, image_queue, hand_queue, mask, tail_queue, 
...
def report(report_hand_hit_queue, report_bounce_queue):
    ...
    # make initial json
    ## make initial hit_region.json
    ## make initial bounce_region.json
    def make_initial_json():
        ...
        for j in [0,5]: # side
            ...
            for h in [0,1,2]: # hand
                ...
                _percentage_hit = np.zeros([1,5])[0]                
...
def upload_report(trigger_report_queue):
    import pyrebase
    config = {
    "apiKey": "AIzaSyAR1TLj8zXo4iLjlUAXS2fpacIqcc9dFUg",
    "authDomain": "volleyai.firebaseapp.com",
    ...
    while cap.isOpened():
        trigger_report_queue.get() # wair for trigger, to handle time sync.
        hit_region = None
...
if __name__ == '__main__':
    ...
    trigger_report_queue = Queue(maxsize=1)
    ...
    Thread(target=video_capture, args=(frame_queue, darknet_image_queue, frame_rgb_queue, pose_queue,
           args.fps_skip, time_queue, args.rotate, bg_mask, frame_queue_toShowing, trigger_report_queue)).start()
    ...
    Thread(target=upload_report, args=(trigger_report_queue,)).start()
