            ...
            for j in [0,5]: # side
                ...
                for h in [0,1,2]: # hand
                    ...
                    if np.sum(_percentage_hit)!=0:
                        ...
                        _percentage_hit = (np.histogram(report_np[_bounce_hand][:,1], bins=10,range=(0,10), density=True))[0]
                        _percentage_hit = _percentage_hit*100
                        # pdb.set_trace()
                    else:
                    ...
                    if j==0:
                        _percentage_hit = [str(int(_p+0.5)) for _p in _percentage_hit[:5]]
                    else:
                        _percentage_hit = [str(int(_p+0.5)) for _p in _percentage_hit[-5:]]
                    _dict_hand_hit[_hand_key_arr[h]] = _percentage_hit
