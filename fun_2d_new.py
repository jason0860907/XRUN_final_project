...
def draw_2d_bg(height, width, win_h, win_w, a_name:str='Player A', b_name:str='Player B', name_dis:int=50, black=False):
    ...
    return img
def draw_2d_demo(height, width, win_h, win_w, a_name:str='Player A', b_name:str='Player B', name_dis:int=50, black=False):
    img = np.zeros([win_h, win_w, 3],dtype=np.uint8)
    dst = get_table_corner_position(height, width, win_h, win_w)
    if not black:
        # cv.rectangle(img, (dst[0][0], dst[0][1]), (dst[2][0], dst[2][1]), (204, 102, 0), -1)
...
def draw_2d_realtime(new_bounce, prev_bounce, hit_point, t_fun, img_2d):
...
def get_heatmap(img_2d, bounce_2d, x0, x1, y0, y1, color_rg ,x_patch=2, y_patch=3):
