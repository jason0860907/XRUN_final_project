...
def drawing(frame_queue, detections_queue, fps_queue, mask, frame_rgb_queue, draw_original,
            time_queue, t_fun, segRally_thres, image_queue, tail_queue):
    noTail_count = 0
    import time
...
def showing(out_filename, image_queue, hand_queue, mask, tail_queue, t_fun, draw_original, name1, name2, watermark_img, frame_queue_toShowing):
    if draw_original:
        h, w, _ = mask.shape
    ...
    while cap.isOpened():
        ...
        if image is not None:  # there is the frame frame video.
            ...
            if args.show_2d:
                ...
                ), tail.get_hit_point_for_draw(), t_fun, img_2d)
...
def pose_estimate(op, opwrapper, frame_list, skeleton_mask, hit_point, hand_queue, left_handed):
    kp_list = []
    for idx, frame in frame_list:
        ...
        kp_list.append((idx, keypoints))
    is_forehand = judge_fore_back(kp_list, hit_point, left_handed)
    is_forehand = 'forehand' if is_forehand else 'backhand'
    hand_queue.put(is_forehand)
            
    
def wait_for_hit_point(op, opwrapper, pose_queue, skeleton_mask, hitpoint_queue, hand_queue, left_handed):
    N = 30
    while cap.isOpened():
        if hitpoint_queue.empty():
            ...
            while not pose_queue.empty():
                ...
                if frame is not None:
                    frame_list.append((idx, frame))
            frame_list = frame_list[-N:]
            
            thread_pose = Thread(target=pose_estimate, args=(op, opwrapper, frame_list, skeleton_mask, hit_point, hand_queue, left_handed))
            thread_pose.start()
...
def judge_fore_back(kp_list, hit_point, left_handed=(False, False)):
    # half_width = width / 2
    half_width = 1920 / 2
...
if __name__ == '__main__':
    ...
    Thread(target=drawing, args=(frame_queue, detections_queue, fps_queue, mask, frame_rgb_queue,
           args.draw_original, time_queue, t_fun, segRally_thres, image_queue, tail_queue)).start()
    Thread(target=showing, args=(args.out_filename, image_queue, hand_queue, mask, tail_queue, t_fun,
           args.draw_original, name1, name2, watermark_img, frame_queue_toShowing)).start()
    Thread(target=wait_for_hit_point, args=(op, opWrapper, pose_queue, skeleton_mask, hitPoint_queue, hand_queue, left_handed)).start()